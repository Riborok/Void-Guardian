#pragma once
#include "..\..\include\sprite\SpriteInfos.hpp"
...
SpriteInfos<StartIndex, Count, Abstract, Info, T0, T1>::SpriteInfos(const ElementAbstractTextureLoadInfoList& list) {
    ...
    for (const auto& [element_type, initializer_info] : list) {
        ...
        _abstract_infos[index] = std::move(initializer_info.abstract_info_vector);
