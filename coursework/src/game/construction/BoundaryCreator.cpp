...
void BoundaryCreator::createBlock(const sf::Vector2f &pos) const {
    _quadtree->insert(_simple_creators->element_creator.create({
        pos, 0, ElementType::BLOCK, _building_info.num, _building_info.boundary_parameters.scale}));
}
...
void BoundaryCreator::createMissedBlock(const int x, const int y, Location* location) const {
    location->addMissedBlock(_simple_creators->element_creator.create({
        ...
        _building_info.boundary_parameters.scale}));
}
...
void BoundaryCreator::createHorBoundary(int start_x, const int last_x, const int y) const {
    while (start_x < last_x) {
        ...
        start_x += _building_info.boundary_parameters.delta.x;
    }
}
...
void BoundaryCreator::createVertBoundary(int start_y, const int last_y, const int x) const {
    while (start_y < last_y) {
        ...
        start_y += _building_info.boundary_parameters.delta.y;
    }
}
...
BoundaryCreator::BoundaryCreator(const BoundaryInfo &building_info, QuadtreeEl &quadtree, SimpleCreators &simple_creators) :
        ...
        _door_size(GeomAuxiliaryFunc::multiplyVectors(building_info.boundary_parameters.door_size_count,
            building_info.boundary_parameters.delta)){ }
...
Location *BoundaryCreator::createLocation(const sf::Vector2i &p0, const sf::Vector2i &p1,
    ...
    const int start_x = p0.x + _building_info.boundary_parameters.delta.x;
    const int start_y = p0.y + _building_info.boundary_parameters.delta.y;
    const int last_x = p1.x - _building_info.boundary_parameters.delta.x;
    const int last_y = p1.y - _building_info.boundary_parameters.delta.y;
...
Location *BoundaryCreator::createLocation(const sf::Vector2i &p0, const int count_x, const int count_y,
        const DoorOpeningMask door_opening, const RoomType room_type) const {
    const sf::Vector2i p1(p0.x + _building_info.boundary_parameters.delta.x * count_x,
        p0.y + _building_info.boundary_parameters.delta.y * count_y);
...
int BoundaryCreator::createVertTransition(const sf::Vector2i &p0, const int last_y) const {
    const int last_x = p0.x + _door_size.x + _building_info.boundary_parameters.delta.x;
    
    createVertBoundary(p0.y, last_y, p0.x);
    createVertBoundary(p0.y, last_y, last_x);
    return last_x + _building_info.boundary_parameters.delta.x;
}

int BoundaryCreator::createHorTransition(const sf::Vector2i &p0, const int last_x) const {
    const int last_y = p0.y + _door_size.y + _building_info.boundary_parameters.delta.y;
    
    createHorBoundary(p0.x, last_x, p0.y);
    createHorBoundary(p0.x, last_x, last_y);
    return last_y + _building_info.boundary_parameters.delta.y;
}

const sf::Vector2i &BoundaryCreator::getDelta() const { return _building_info.boundary_parameters.delta; }

const sf::Vector2i &BoundaryCreator::getDoorSizeCount() const { return _building_info.boundary_parameters.door_size_count; }