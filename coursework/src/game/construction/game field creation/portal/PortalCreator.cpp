...
void PortalCreator::spawnPortal(const sf::Vector2f& p, const float angle, const PortalData& portal_data) const {
...
void PortalCreator::spawnInPortal(const sf::Vector2f& p, const float angle) const {
    spawnPortal(p, angle, _in_out_portals->in_portal);    
}

void PortalCreator::spawnOutPortal(const sf::Vector2f& p, const float angle) const {
    spawnPortal(p, angle, _in_out_portals->out_portal);
}