...
void EntityDamageManager::applyDamage(const Bullet &bullet, const ElementCollisionSet &hits) const {
    for (const auto* hit : hits) {
        ...
        case ElementType::CHARACTER: {
            const auto& player_map = _entity_maps->player_map.getMap();
            const auto& enemy_map = _entity_maps->enemy_map.getMap();
            if (const auto it_pl = player_map.find(id); it_pl != player_map.end())
                applyDamage(bullet.getBulletCasing().getStats(), it_pl->second);
            else if (const auto it_en = enemy_map.find(id); it_en != enemy_map.end())
