...
GameLoop::GameLoop(sf::RenderWindow& window, InputHandler& input_handler, HotkeyManager& hotkey_manager, GameUpdater &game_updater)
    : _event_manager(window, input_handler, hotkey_manager, _game_loop_state, game_updater.getWindowParam()),
    _sprite_drawer(window), _game_updater(&game_updater) { }
