...
#include "../../../include/gui/UIController/SettingsController.hpp"
#include "../../../include/additionally/AdditionalFunc.hpp"
#include "../../../include/additionally/PixelConverter.hpp"
#include "../../../include/gui/ReservedKeys.hpp"

float SettingsController::getMaxTextWidth() const {
    float result = 0;
    for (const auto& text : _settings_controller_info.texts)
        if (const float width(text.getGlobalBounds().width); result < width)
            result = width;
    return result;
}

void SettingsController::setPositions() {
    ...
    const size_t count = _settings_controller_info.texts.size();
    for (size_t i = 0; i < count; ++i) {
        _settings_buttons.setPos(i, {button_x, curr_y});
        _settings_controller_info.texts[i].setPosition({text_x, curr_y});
        curr_y += button_spacing;
    }
    _settings_buttons.setPos(count, {x_center, curr_y});
}

void SettingsController::handleKeyPressed(const sf::Keyboard::Key key) {
    switch (key) {
    ...
    default:
        if (_settings_buttons.hasActiveButton()) {
            _settings_buttons.setInputInfo(InputInfo{key});
            drawSettings();
        }
        break;
    }
...
void SettingsController::createSettings(const ButtonColors& button_colors) {
    _settings_buttons.createButtons(_continue_settings, button_colors);
    
    for (auto& text : _settings_controller_info.texts)
        text.setFillColor(button_colors.text_color);
}

void SettingsController::drawSettings() {
...
void SettingsController::handleEvents() {
    auto& window = _game_context->window;
    sf::Event event;
    while (window.pollEvent(event)) {
        ...
        case sf::Event::MouseButtonReleased:
            if (_settings_buttons.hasActiveButton()) {
                _settings_buttons.setInputInfo(InputInfo{event.mouseButton.button});
                drawSettings();
            }
...
SettingsController::SettingsController(GameContext& game_context, const sf::Font &font, const Cursors &cursors,
    const SettingColors &setting_colors): _game_context(&game_context),
        _settings_controller_info(font),
...
void SettingsController::startSettings() {
