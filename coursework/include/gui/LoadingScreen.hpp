...
class LoadingScreen final {
    ...
    void setTextStr();
public:
    LoadingScreen(sf::RenderWindow &window, FullscreenToggler &fullscreen_toggler, LoadingScreenInfo &&loading_screen_info, const Colors &colors);
    void start();
    void end();
    
    ~LoadingScreen() noexcept = default;
    LoadingScreen(LoadingScreen&&) noexcept = delete;
